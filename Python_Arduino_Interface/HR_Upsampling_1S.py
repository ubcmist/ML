...
Input_HR_csv = args['input']
output_HR_csv = args['output']
resampling_type = args['type']
collection_Date = args['date']
# Input_HR_csv = 'Data/VideoGames_Apex_HR_GSR_Hooman/HR/20190322_x.csv'
# output_HR_csv = 'Data/VideoGames_Apex_HR_GSR_Hooman/HR_UpSampled_1S/20190322_x.csv'
...
HR_original = read_csv(Input_HR_csv, header=0, parse_dates=[0], index_col=0, squeeze=True,  date_parser=date_time_parser)
resampled = HR_original.resample('1S')
if resampling_type == "linear":
    interpolated = resampled.interpolate(method='linear')
    rounded = interpolated.round()
...
if output_HR_csv is None:
    output_HR_csv_rounded = osp.join(osp.dirname(Input_HR_csv) + '_UpSampled_1S', osp.basename(Input_HR_csv))
    output_HR_csv_NotRounded = osp.join(osp.dirname(Input_HR_csv) + '_UpSampled_1S_NotRounded', osp.basename(Input_HR_csv))
if not osp.exists(osp.dirname(output_HR_csv_rounded)):
    os.makedirs(osp.dirname(output_HR_csv_rounded))
rounded.to_csv(output_HR_csv_rounded[:-4] + "_" + resampling_type +'.csv' , header =True, date_format='%Y-%m-%d-%H:%M:%S')
if not osp.exists(osp.dirname(output_HR_csv_NotRounded)):
    os.makedirs(osp.dirname(output_HR_csv_NotRounded))
interpolated.to_csv(output_HR_csv_NotRounded[:-4] + "_" + resampling_type +'.csv' , header =True, date_format='%Y-%m-%d-%H:%M:%S')